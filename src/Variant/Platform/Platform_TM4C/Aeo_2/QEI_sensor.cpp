...
//	GPIOPinConfigure(GPIO_PF4_IDX0);
    ...
    static void ConfigureQei1(uint32_t nConfig) {
        ...
        // GPIO config
        GPIOPadConfigSet(GPIO_PORTC_BASE, GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6, GPIO_STRENGTH_2MA,
    ...
    bool QEI_sensor::GetIndex() {
        switch (m_nDevice) {
            ...
            case QEI1:
                return GPIOPinRead(GPIO_PORTC_BASE, GPIO_PIN_4) != 0;
            default:
                return false;
        }
    }

    int32_t QEI_sensor::get_position() const {
    ...
    void QEI_sensor::Set(int32_t nValue) const {
        uint32_t nPosition;
        if (0 > nValue) {
            nPosition = nValue;
        } else {
